...
<?php
    ...
    if(isset($_POST['submit_one'])){
         ...
         if($evaluate==null){
           ...
           header("Location: ../html/iframe_inv/pre-charge-archive_search.php");
           exit();
         }else{

        $pulis_details = "SELECT * from viewthings where status_given='Precharge Archive' &&(com_first like '%$evaluate%' || com_last like '%$evaluate%' || pulis_first like '%$evaluate%' || pulis_last like '%$evaluate%' || official_docket like '%$evaluate%' || unit_base like '%$evaluate%' || status_given like '%$evaluate%' || official_docket like '%$evaluate%')";
        $pd = $conn->query($pulis_details);


if($pd->num_rows > 0){

echo "<form action='pre-charge-inves-archive.inc.php' method='POST' style='float:right; margin-right:20px;'>";

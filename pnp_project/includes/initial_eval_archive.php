...
<?php
    ...
    if(isset($_POST['submit_one'])){
         ...
         if($evaluate==null){
           ...
           echo "asdfklalksdfkaklsdfkasdf";

           header("Location: ../html/iframe_inv/initial-eval-archive_search.php");
           exit();
         }else{

        $pulis_details = "SELECT * from viewthings where status_given='Initial Evaluation Archive' &&(com_first like '%$evaluate%' || com_last like '%$evaluate%' || pulis_first like '%$evaluate%' || pulis_last like '%$evaluate%' || official_docket like '%$evaluate%' || unit_base like '%$evaluate%' || status_given like '%$evaluate%' || official_docket like '%$evaluate%')";
        $pd = $conn->query($pulis_details);


if($pd->num_rows > 0){

echo "<form action='initial_eval_archive.php' method='POST' style='float:right; margin-right:20px;'>";
...
echo"<table  class='table table-striped table-hover' style='float:left;'>";
  ...
  echo"<th colspan=2 >Police Name</th><th>Unit</th><th colspan=2>complainant</th><th>Date of cognitance</th><th>Case Close Date</th><th colspan=2>Investigator on case</th><th>Status</th><th>Docket No.</th>";
  echo"<tr>";
